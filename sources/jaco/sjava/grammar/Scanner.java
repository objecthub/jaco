...
public class Scanner extends jaco.java.grammar.Scanner {
    ...
    public Scanner(SJSyntacticContext context, Sourcefile source) {
    ...
    protected void initKeywords() {
        ...
        enterKeyword("select", Symbols.SELECT); 
	enterKeyword("selectnowait", Symbols.SELECTNOWAIT);
        enterKeyword("accept", Symbols.ACCEPT);
        enterKeyword("from", Symbols.FROM);
        enterKeyword("waituntil", Symbols.WAITUNTIL);       
        enterKeyword("cases", Symbols.CASES);// DUY 24.11.01: add new keyword for Selectcase
        enterKeyword("when", Symbols.WHEN);
        enterKeyword("active", Symbols.ACTIVE);
        if (asserts)

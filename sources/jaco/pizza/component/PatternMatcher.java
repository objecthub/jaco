            ...
            switch (tree) {
                ...
                case VarDecl(Name name, _, Tree vartype, _, VarDef v):
                    ...
                    if (definitions.isCase(vartype.type.tdef()) &&
                        ...
                        return vartype.type.tdef().owner.type;
                    else                                // (type v)
                ...
                case Select(_, _, _):                  // pattern without args
                case Ident(_, _):
                    return types.isAlgebraicType(pt) ? tree.def().owner.type : tree.type;
                default:
        ...
        if (tree.type.isTypeOfConstant())
            ...
            switch (tree)
            {
                ...
                case VarDecl(Name name, _, Tree vartype, _, VarDef v):
                    ...
                    if (types.isCaseType(vartype.type) &&
                        types.isAlgebraicType(header.type))
                    {
						int tag;
                        if ((env != null) && ((v.modifiers & USED) != 0))
                            env.newBoundCastedVar(tree.pos, name, vartype.type, header);
                        tag = definitions.getCaseTag(vartype.type.tdef());
                        //System.out.println("line = " + Position.line(vartype.pos) + "; " + vartype.type + " / " + header.type + " / " + tag + " / " + definitions.getCaseConstr(header.type.tdef(), tag));//DEBUG
                        return make.ConstrPat(tree.pos,
                            vartype.type,
                            tag = definitions.getCaseTag(vartype.type.tdef()),
                ...
                case Select(_, _, _):                  // pattern without args
                case Ident(_, _):
                    if (types.isAlgebraicType(header.type))
                        return make.ConstrPat(
                                tree.pos, tree.def().type,
                                definitions.getCaseTag(tree.def()), 0);
                    else
                        return make.VariablePat(tree.pos, tree);
    ...
    protected boolean samePat(PatternNode p, PatternNode q)
    {
        switch (p)
        {
            ...
            case VariablePat(Tree tree):
                switch (q)
                {
                    case VariablePat(Tree qtree):
                        return tree.def() == qtree.def();
                }
                return false;
        ...
        while (patNode != null)
            switch (patNode)
            {
                        ...
                        result = trees.append(result,
                            trees.at(patNode.pos).make(
                                genVarDecl(idname, types.intType,
                                    newdef.Apply(
                                        newdef.Select(
                                            trees.copy(sel), EACConst.GET_SUB_ID_N), trees.noTrees),
                                    types.intType)));
                        Slot s0 = null;
                        while (s != null) {
                            if (types.sametype(s.guard, patNode.type))
                                ...
                                result = trees.append(result,
                                    trees.at(s.pos).make(
                                        newdef.If(newdef.Binop(EQ, newdef.Ident(idname), newdef.Select(trees.toTree(s.guard), EACConst.SUB_ID_N)),
                ...
                case SwitchHeader(Tree sel, _, SwitchHeader next):
                    ...
                    //System.out.println("ALGEBRAIC TYPE: " + patNode.type);
...
//////////// environment for cases
    ...
    protected class CaseEnv
    {
        ...
        public void newBoundCastedVar(int pos, Name name, Type type, SwitchHeader sh)
        {
            if (numVars == boundVars.length)
            {
                VarDecl[]   newVars = new VarDecl[numVars * 2];
            ...
            boundVars[numVars++] = (VarDecl)trees.make.VarDecl(name,
                                        0, typeToTree(type),
                                        trees.make.Ident(sh.casted)).at(pos).setType(sh.type);
        }

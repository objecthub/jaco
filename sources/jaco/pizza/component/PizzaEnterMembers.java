        ...
        main: switch (tree)
        {
            ...
            case VarDecl(Name name, int mods, Tree vartype, _, _):
                    ...
                    ((mods & STATIC) == 0))
                {
                    ...
                    result = super.memberEnter(tree, env);
                if (result instanceof VDef)
                    ((VDef)result).metaData = (String)((PizzaTrees)trees).metaData.get(tree);
                break;
            
            case MethodDecl(Name name, int mods, Tree restype, VarDecl[] params,
                            Tree[] thrown, Tree[] stats, _):
                result = super.memberEnter(tree, env);
                if (result instanceof MDef)
                    ((MDef)result).metaData = (String)((PizzaTrees)trees).metaData.get(tree);

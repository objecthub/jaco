//      /   _ _      JaCo
//  \  //\ / / \     - the lalr(1) parser framework used by jcup parsers
//   \//  \\_\_/     
//         \         Matthias Zenger, 19/02/99
 ...
 */
    public ParseStack stack = new ParseStack();
...
/** the action object
 */
    protected ActionInterface   actObj;
    

/** find the parser tables file in the classpath
 */
    protected static InputStream find_tables(String filename) throws IOException
    {
        String      pathSep = System.getProperty("path.separator");
        String      classPath = System.getProperty("java.class.path") + pathSep;
        int         i = 0;
        VirtualFile f = null;
        filename = filename.replace('.', File.separatorChar) + ".tables";
        while ((i < classPath.length()) && (f == null))
        {
            int     end = classPath.indexOf(pathSep, i);
            String  dirname = classPath.substring(i, end);
            f = VirtualFile.open(dirname, filename);
            i = end + 1;
        }
   ...
   */
    public Symbol parse() throws java.lang.Exception
    {
        ...
        while (!_done_parsing)
        {
            ...
            if ((act = get_action(stack.stack[tos].parse_state, cur_token.sym)) > 0)
            {
            ...
            if (act < 0)
            {
                ...
                lhs_sym = actObj.doAction(act, tos);
                
                /* look up information about the production */
                
                /* pop the handle off the stack */
                stack.sp -= (handle_size = production_tab[act][1]);
                tos -= handle_size;
                
                /* look up the state to go to from the one popped back to */
                lhs_sym.parse_state = get_reduce(stack.stack[tos].parse_state, production_tab[act][0]);
  ...
  public Symbol debug_parse()
      ...
      for (_done_parsing = false; !_done_parsing; )
    {
      ...
      /* if its less than zero, then it encodes a reduce action */
        ...
        {
          /* perform the action for the reduce */
          lhs_sym = actObj.doAction((-act)-1, tos);

...
public class production {
  ...
  public production(
    ...
    if (rhs_parts[rhs_l - 1].is_action()) {
      ...
      String[][] declare_lft = declare_lefts(rhs_parts, rightlen);
    ...
    6/14/96 frankf */
      ...
      {
          for (int j = 0; j < declare_lft.length; j++)
          {
            if ((declare_lft[j][0] != null) && (tail_action.code_string().indexOf(declare_lft[j][0]) >= 0))
              action_str = action_str + declare_lft[j][1];
          }
  ...
  protected String make_declaration(
                    ...
                    boolean left)
    {
      ...
      if (emit.lr_values() && left)
      {
        ret = "\t\tint " + labelname + "left = $stack[" + emit.pre("top") + 
                ((offset != 0) ? ("-" + offset) : "") + "].left;\n";
// MZ: is it possible to make the next statement optional, depending
//     on a switch? I think, for most purposes, a left position will
//     be enough
    /*
        ret += "\t\tint " + labelname + "right = $stack[" + emit.pre("top") +
                ((offset != 0) ? ("-" + offset) : "") + "].right;\n";
    */
      }
      else ret = "";
      ...
      if (!left)
        ret = ret + "\t\t" + stack_type + " " + labelname + " = (" + stack_type + 
              ")$stack[" + emit.pre("top") + ((offset != 0) ? ("-" + offset) : "") + "].value;\n";
      return ret;
    }
  /*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/
  ...
  protected String[][] declare_lefts(production_part rhs[], int rhs_len)
  {
      String[][]    decl = new String[rhs_len][2];
      int           i = 0;
        ...
        for (int pos = 0; pos < rhs_len; pos++)
        {
            if (!rhs[pos].is_action())
            {
                ...
                if (part.label() != null)
                {
                    decl[i][0] = part.label() + "left";
                    decl[i][1] = make_declaration(part.label(), part.the_symbol().stack_type(), 
                                                    rhs_len-pos-1, true);
                    i++;
                }
            }
  ...
  protected void remove_embedded_actions() throws internal_error
    ...
    if (rhs(act_loc).is_action())
      {
        declare_str = declare_labels(_rhs, act_loc, null);
        String[][]  decls = declare_lefts(_rhs, act_loc);
        
        /* create a new non terminal for the action production */
        new_nt = non_terminal.create_new();
        
        String      code_str = ((action_part)rhs(act_loc)).code_string();
        for (int j = 0; j < decls.length; j++)
        {
            if ((decls[j][0] != null) && (code_str.indexOf(decls[j][0]) >= 0))
              declare_str = declare_str + decls[j][1];
        }
        
        /* create a new production with just the action */
        new_prod = new action_production(this, new_nt, null, 0, declare_str + code_str);
